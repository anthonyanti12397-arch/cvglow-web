...
// Classic — original clean serif layout
export default function TemplateClassic({ content }: { content: ResumeContent }) {
  return (
    <div className="bg-white text-gray-900 p-8 min-h-[1000px]" style={{fontFamily: "Georgia, serif"}}>
      <div className="border-b-2 pb-5 mb-5" style={{borderColor: "#0A1628"}}>
        <h1 className="text-3xl font-bold text-gray-900 mb-1">{content.fullName || 'Your Name'}</h1>
        {content.jobTitle && <p className="text-lg font-medium mb-2" style={{color: "#0A1628"}}>{content.jobTitle}</p>}
              ...
              <div key={i}>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="text-sm font-medium" style={{color: "#0A1628"}}>{exp.company}</div>
                  </div>
          ...
          <div className="space-y-3">
            {content.educations.map((edu, i) => (
              <div key={i} className="flex items-start justify-between">
                ...
                <div className="text-sm text-gray-400 shrink-0 ml-4">{edu.graduateYear && `Class of ${edu.graduateYear}`}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {content.skills.map((skill, i) => (
              <span key={i} className="text-sm px-3 py-1 rounded-full border" style={{borderColor: "#ddd", color: "#444"}}>
                {skill}
              </span>
            ))}
          </div>
